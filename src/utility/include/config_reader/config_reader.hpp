...
public:
    ...
    std::vector<int> GetVectorInt(std::string name, std::vector<int> default_value){
        std::string valstr = Get(name, "");
        if(valstr == ""){
            return default_value;
        }
        ...
        int prev_pos = 0;
        while (valstr != ""){
            std::size_t found = valstr.find("-");
            if (found != std::string::npos){
                std::string str_seg = valstr.substr(prev_pos, found);
                v.push_back(stoi(str_seg));
                valstr = valstr.substr(found+1);
            }
            else{
                std::string str_seg = valstr.substr(prev_pos);
                v.push_back(stoi(str_seg));
                valstr = "";
            }
        }
    ...
    Eigen::MatrixXi GetVectorBinary(std::string name, Eigen::MatrixXi default_value){
        std::vector<int> vv = GetVectorInt(name,{});
        Eigen::MatrixXi res = Eigen::MatrixXi::Ones(1,vv.size());
    ...
    std::string GetString(std::string name, std::string default_value){
        std::string valstr = Get(name, "");
        if(valstr == ""){
            return default_value;
        }
