...
{   
    ...
    while(flag_IPAS != true){
        ...
        for (int i = 0; i < 3; i++){
            std::cout << "Vehicle " << i << "'s path is: ";
            for (auto kk: vehicle_team_->auto_team_[i]->vehicle_.task_path_){
                std::cout << kk << ", ";
            }
            std::cout << std::endl;
            std::cout << "Vehicle " << i << "'s path is: ";
            for (auto vt: path_ltl_[i]){
                std::cout << vt->id_ << ", ";
            }
        }
        std::cout << std::endl;
        ...
        std::vector<int64_t> local_hspots; // For each actor
        for(auto agent: vehicle_team_->auto_team_){
        ...
        // hot spots after consensus phase
