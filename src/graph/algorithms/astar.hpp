...
class AStar
{
  ...
  public:
	...
	static Path_t<StateType> Search(std::shared_ptr<Graph_t<StateType, TransitionType>> graph, int64_t start_id, int64_t goal_id, CalcHeuristicFunc_t<StateType, TransitionType> calc_heuristic)
	{
	...
	template <typename StateType, typename TransitionType>
	static Path_t<StateType> SearchIgnoreEdgeCost(std::shared_ptr<Graph_t<StateType, TransitionType>> graph, int64_t start_id, int64_t goal_id, CalcHeuristicFunc_t<StateType, TransitionType> calc_heuristic)
	{
		// reset last search information
		graph->ResetGraphVertices();
		...
		// start a new search and return result
		if (start != nullptr && goal != nullptr)
			return SearchIgnoreEdgeCost(start, goal, calc_heuristic);
		else
			return empty;
	}

	template <typename StateType, typename TransitionType>
	static Path_t<StateType> Search(Graph_t<StateType, TransitionType> *graph, int64_t start_id, int64_t goal_id, CalcHeuristicFunc_t<StateType, TransitionType> calc_heuristic)
	{
		// reset last search information
		graph->ResetGraphVertices();
			...
			for (auto &edge : current_vertex->edges_to_)
			{
				...
				if (successor->is_checked_ == false)
				{
					...
					if (successor->is_in_openlist_ == false || new_cost < successor->g_cost_)
					{
						...
						successor->f_astar_ = successor->g_cost_ + successor->h_astar_;
	...
	template <typename StateType, typename TransitionType>
	static Path_t<StateType> ProductIncSearch(std::shared_ptr<Graph_t<ProductState*>> product_graph, int64_t start_id, std::vector<int64_t> buchi_acc, GetNeighbourFunc_t<StateType, TransitionType>  get_neighbours)
	{
		using GraphVertexType = Vertex_t<StateType, TransitionType>;

		// create a new graph with only start and goal vertices
		Graph_t<StateType, TransitionType> graph;
		GraphVertexType *start_vtx = product_graph->GetVertexFromID(start_id);
		GraphVertexType *goal_vtx;

		//for(auto &goal: goal_id)
		//	GraphVertexType *goal_state = graph.GetVertexFromID(goal);
		...
		while (!openlist.empty() && found_path != true)
		{
			...
			for (auto &nb : neighbours)
				product_graph->AddEdge(current_vertex->state_, std::get<0>(nb), std::get<1>(nb));
			...
			for (auto &edge : current_vertex->edges_to_)
			{
				...
				if (successor->is_checked_ == false)
				{
					...
					if (successor->is_in_openlist_ == false || new_cost < successor->g_astar_)
					{
						...
						auto found = std::find(buchi_acc.begin(),buchi_acc.end(), successor->state_->buchi_vertex_->state_->id_);
						if (found != buchi_acc.end())
						{
							found_path = true;
							goal_vtx = successor;
						}
		...
		if (found_path)
		{
			//std::cout << "path found with cost " << std::endl;
			auto path_vtx = ReconstructPath(start_vtx, goal_vtx);
			for (const auto &wp : path_vtx)
			 	path.push_back(wp->state_);
			
			std::cout << "Path lentgh is " << path.size() << std::endl;
		}
		else{
			std::cout << "failed to find a path" << std::endl;
			path = {};
	...
	template <typename StateType, typename TransitionType>
	static Path_t<StateType> ProductIncSearchCBTA(std::shared_ptr<Graph_t<ProductState*>> product_graph, int64_t start_id, std::vector<int64_t> buchi_acc, GetNeighbourFunc_t<StateType, TransitionType>  get_neighbours)
	{
		using GraphVertexType = Vertex_t<StateType, TransitionType>;
		...
		// Find the set of start vertices connect to the virtual start
		std::vector<GraphVertexType *> start_vertex = start_vtx->GetNeighbours();
  ...
  private:
	...
	template <typename StateType, typename TransitionType>
	static Path_t<StateType> SearchIgnoreEdgeCost(Vertex_t<StateType, TransitionType> *start_vtx, Vertex_t<StateType, TransitionType> *goal_vtx, std::function<TransitionType(StateType, StateType)> CalcHeuristic)
	{
		using GraphVertexType = Vertex_t<StateType, TransitionType>;
		...
		while (!openlist.empty() && found_path != true)
		{
			...
			for (auto &edge : current_vertex->edges_to_)
			{
				...
				if (successor->is_checked_ == false)
				{
					auto new_cost = current_vertex->g_astar_ + 1.0;
