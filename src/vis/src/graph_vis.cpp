...
cv::Scalar GraphVis::v10_color_ = Scalar(250,128,114);
cv::Scalar GraphVis::v11_color_ = Scalar(176,196,222);
cv::Scalar GraphVis::v12_color_ = Scalar(47,79,79);
cv::Scalar GraphVis::v13_color_ = Scalar(75,0,130);
...
void GraphVis::VisSquareGrid(const SquareGrid& grid, cv::OutputArray _dst)
{
	...
	for(auto &gird_col: grid.grid_cells_)
	{
		for (auto &itc: gird_col){
			if((*itc).occupancy_ == OccupancyType::OCCUPIED)
				//FillSquareCellColor((*itc).second->bbox_, obs_color_, dst);
				VisUtils::FillRectangularArea(dst, (*itc).bbox_, ln_color_);
			else if((*itc).occupancy_ == OccupancyType::INTERESTED)
				//FillSquareCellColor((*itc).second->bbox_, aoi_color_, dst);
				VisUtils::FillRectangularArea(dst, (*itc).bbox_, aoi_color_);
			else if((*itc).occupancy_ == OccupancyType::UNKNOWN)
				VisUtils::FillRectangularArea(dst, (*itc).bbox_, unkn_color_);
...
void GraphVis::VisSquareGridPath(Path_t<SquareCell *>& path, cv::InputArray _src, cv::OutputArray _dst, int64_t idx)
{
	...
	uint64_t x,y;
	x = cell_s->position_.x;
	x = x - (cell_s->bbox_.x.max - cell_s->bbox_.x.min)/30;
	y = cell_s->position_.y;
	y = y + (cell_s->bbox_.y.max - cell_s->bbox_.y.min)/30;
	//FillSquareCellColor(cell_s->bbox_, start_color_, dst);
	...
	switch(idx){
		...
		case 10: vehicle_color = v10_color_; vehicle_idx = "V11"; d = -5; break;
		case 11: vehicle_color = v11_color_; vehicle_idx = "V12"; d = 5; break;
		case 12: vehicle_color = v12_color_; vehicle_idx = "V13"; d = 15; break;
		case 13: vehicle_color = v13_color_; vehicle_idx = "V14"; d = -15; break;
	}
