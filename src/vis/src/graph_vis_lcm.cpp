...
void GraphVisLCM::TransferRandomInfoLCM(std::shared_ptr<Graph_t<SquareCell *>> graph, std::map<int64_t, Path_t<SquareCell*>> paths, std::vector<int64_t> sensors_pos, TasksList tasks, std::vector<Agent> agents){
    srand(time(NULL));
    lcm::LCM lcm;
    ...
    // Define Agents
    random_graph_data::agents_data agents_data_;
    agents_data_.num_agents_ = agents.size();
    for(auto & agent: agents){
        random_graph_data::agent_data agent_data_;
        agent_data_.init_pos_ = agent.init_pos_;
        agent_data_.idx_ = agent.idx_;

        agents_data_.agents_.push_back(agent_data_);
    }
    

    // Define Tasks
    random_graph_data::tasks_data tasks_data_;
    tasks_data_.num_tasks_ = tasks.tasks_.size();
    for(int ts = 0; ts < tasks.tasks_.size(); ts++){
        random_graph_data::task_data task_data_;
        task_data_.idx_ = ts + 3;
        task_data_.target_pos_ = tasks.tasks_[ts].pos_.front(); 
        tasks_data_.tasks_.push_back(task_data_);
    }
    ...
    std::string Graph_data = std::to_string(fig_idx_) + "GraphData";
    std::string Path_Collection_data = std::to_string(fig_idx_) + "PathCollectionData";
    std::string Agents_data_channel = std::to_string(fig_idx_) + "AgentsData";
    std::string Tasks_data_channel = std::to_string(fig_idx_) + "TasksData";

    

    lcm.publish(Graph_data, &map_data_);
    lcm.publish(Path_Collection_data, &paths_data_);
    lcm.publish(Agents_data_channel, &agents_data_);
    lcm.publish(Tasks_data_channel, &tasks_data_);
