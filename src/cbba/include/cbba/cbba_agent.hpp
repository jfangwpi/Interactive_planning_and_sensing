...
public:
	...
	Agent(int id, int startID, int num_tasks_inde, int num_tasks_de, int num_agents, int num_sensors);
	// Destructor
	~Agent(){};
...
public:
	...
	/* Information Gain - Decentralized system */
	int64_t num_sensors_;
	std::vector<std::pair<int,double>> hotspots_;
...
public:
	...
	void bundle_add(TasksList tasks, const std::shared_ptr<Graph_t<SquareCell*>> graph, bool flag_measurement=false);
	...
	void reward_update_measurement(TasksList tasks_list, const std::shared_ptr<Graph_t<SquareCell*>> graph);
	...
	void ComputeHotSpotsFromLocalInfo(TasksList tasks);
	void IpasConvergenceCheck(TasksList tasks);
	void SetLocalMap(std::shared_ptr<SquareGrid> grid, std::shared_ptr<Graph_t<SquareCell*>> graph);
...
namespace TaskAssignment
{
	...
	std::vector<Agent> InitializeAgents(int64_t num_sensors);
	...
	/*** Functions for CBBA ***/
	void bundle_add(TasksList tasks, const std::shared_ptr<Graph_t<SquareCell*>> graph, std::vector<Agent>& agents, bool flag_measurement=false);
	...
	// Compute the distance with uncertainty betweem two nodes.	
	double CalcHeuristicUncertain(SquareCell *node1, SquareCell *node2);

	/*** Reset before Task Assignment for the group of agents ***/
	void ResetTaskAssignmentInfo(std::vector<Agent>& agents);

	// Decompose the current routes into a list of segements
    std::vector<Path_t<SquareCell*>> PathsDecomposition(TasksList tasks, std::vector<Agent>& agents, std::map<int64_t,Path_t<SquareCell*>> paths);
};
