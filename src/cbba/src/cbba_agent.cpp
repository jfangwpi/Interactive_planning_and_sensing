...
Agent::Agent(int id, int startID, int num_tasks_inde, int num_tasks_de, int num_agents)
{
	idx_ = id;
	init_pos_ = startID;
	comm_topo_ = Eigen::MatrixXd::Ones(1, num_agents);
	iter_ = 0;
	cbba_bundle_ = {};
	cbba_path_ = {};
...
Agent::Agent(int id, int startID, int num_tasks_inde, int num_tasks_de, int num_agents, int num_sensors){
	Agent(id,startID,num_tasks_inde,num_tasks_de,num_agents);
	num_sensors_ = num_sensors;
	
};

void Agent::SetLocalMap(std::shared_ptr<SquareGrid> grid, std::shared_ptr<Graph_t<SquareCell*>> graph){
	local_grid_ = grid->DuplicateSquareGrid();
	local_graph_ = GridGraph::BuildGraphFromSquareGrid(local_grid_,false,true);
	std::cout << "The local information of map is done. " << std::endl; 
}
...
// Initialize agents system with decentralized network
std::vector<Agent> TaskAssignment::InitializeAgents(int64_t num_sensors){
	std::vector<Agent> agents = InitializeAgents();
	for(auto&ag: agents){
		ag.num_sensors_ = num_sensors;
	}
	return agents;
}
...
double TaskAssignment::PathLengthCalculationBasedType(TasksList tasks_list, std::shared_ptr<Graph_t<SquareCell*>> graph, std::vector<int> bundle, int start_idx_){
	...
	for (int k = 0; k < pos_seq.size(); k++){
		...
		path_part = AStar::Search(graph,start_node->state_->id_,finish_node->state_->id_, CalcHeuristicFunc_t<SquareCell *>(TaskAssignment::CalcHeuristicUncertain));
		if (k == pos_seq.size() - 1){
			path.insert(path.end(), path_part.begin(), path_part.end());
	...
	for(int i = 0; i < path.size()-1; i++){
		double l = TaskAssignment::CalcHeuristicUncertain(path[i],path[i+1]);
		path_length += l;
	}
...
void Agent::reward_update(TasksList tasks_list, const std::shared_ptr<Graph_t<SquareCell*>> graph){
	...
	for (auto &tk: tasks_list.tasks_){
		if (tk.num_agents_ == 1){
			tasks_NotInBundle.push_back(tk.idx_);
		}
	}
	...
	// std::cout << cbba_reward_ << std::endl;
...
void Agent::reward_update_measurement(TasksList tasks_list, const std::shared_ptr<Graph_t<SquareCell*>> graph){
	std::vector<int> tasks_NotInBundle = {};
	for (auto &tk: tasks_list.tasks_){
		if (tk.num_agents_ == 1){
			tasks_NotInBundle.push_back(tk.idx_);
...
void Agent::bundle_add(TasksList tasks, const std::shared_ptr<Graph_t<SquareCell*>> graph, bool flag_measurement){
	bool bundleFull = -1;
	...
	while (bundleFull == 0){
		if (flag_measurement == true){
			reward_update_measurement(tasks,graph);
		}
		else{
...
void TaskAssignment::bundle_add(TasksList tasks, const std::shared_ptr<Graph_t<SquareCell*>> graph, std::vector<Agent>& agents, bool flag_measurement){
	for (auto &agent: agents){
		agent.bundle_add(tasks, graph,flag_measurement);
	}
};
...
// Update the information gain based on local graph and select the hotspots with maximum ig
void Agent::ComputeHotSpotsFromLocalInfo(TasksList tasks){
	...
	if(cbba_path_.empty()){
		...
		int64_t start_id = init_pos_;
		for(auto&tt: cbba_path_){
			cbba_Task tk = tasks.FindTaskFromID(tt);
			std::vector<int64_t> target_ids = tk.pos_;
			for(auto&kk: target_ids){
				int64_t tg_id = kk;
				Path_t<SquareCell*> path_seg = AStar::Search(local_graph_,start_id,tg_id,CalcHeuristicFunc_t<SquareCell *>(GridGraph::CalcHeuristic));
	...
	IpasConvergenceCheck(tasks);
	if (flag_ipas_ == true){
		std::cout << "IPAS convergence is achieved." << std::endl;
	}
	else{
		// Find the HotSpots
...
void Agent::IpasConvergenceCheck(TasksList tasks){
	if (cbba_path_.empty()){
		flag_ipas_ = true;
	}
	else{
		Path_t<SquareCell*> path = GridGraph::PathComputationAStar(tasks,local_graph_,cbba_path_,init_pos_);
		double en_thre = path.size() * entropy_threshold;
        double entropy_path = GridGraph::EntropyPath(path, local_graph_);
...
double TaskAssignment::CalcHeuristicUncertain(SquareCell *node1, SquareCell *node2)
{
    double dist = 1.0*(1.0 - node2->p_) + 200.0 * node2->p_;
    return dist;
}


void TaskAssignment::ResetTaskAssignmentInfo(std::vector<Agent>& agents){
	for (auto &agent:agents){
		agent.cbba_bundle_ = {};
		agent.cbba_path_ = {};

		agent.iteration_neighbors_ = Eigen::MatrixXd::Zero(1, agent.num_agents_);
		agent.history_.iter_neighbors_his = {agent.iteration_neighbors_};
		
		agent.cbba_y_ = Eigen::MatrixXd::Zero(1, agent.num_tasks_inde_);
		agent.cbba_z_ = -1 * Eigen::MatrixXd::Ones(1, agent.num_tasks_inde_);

		agent.history_.y_history = {agent.cbba_y_};
		agent.history_.z_history = {agent.cbba_z_};
		
		agent.cbba_reward_ = -1 * Eigen::MatrixXd::Ones(1, agent.num_tasks_);
		// syn_c_ = -1 * Eigen::MatrixXd::Ones(1, num_tasks_);
		agent.insert_pos_ = -1 * Eigen::MatrixXd::Ones(1, agent.num_tasks_);
	}
}


std::vector<Path_t<SquareCell*>> TaskAssignment::PathsDecomposition(TasksList tasks, std::vector<Agent>& agents, std::map<int64_t,Path_t<SquareCell*>> paths){
	// The ROI is the target position vehicle has to visit
    // Decompose the current routes based on task assignment
    std::vector<Path_t<SquareCell*>> paths_collection;
    for (auto& agent: agents){
        if(agent.cbba_path_.size() == 1){
            paths_collection.push_back(paths[agent.idx_]);
        }
        else{
            int64_t begining_idx = 0;
            std::vector<int64_t> path_id_ = {};
            for(auto &cell_id: paths[agent.idx_]){
                path_id_.push_back(cell_id->id_);
            }
            for(auto&task_id: agent.cbba_path_){
                cbba_Task task = tasks.FindTaskFromID(task_id);
                std::vector<int64_t> targets_pos_ = task.pos_;
                for (auto& pos: targets_pos_){
                    std::vector<int64_t>::iterator it = std::find(path_id_.begin(), path_id_.end(), pos);
                    if(it == path_id_.end()){
                        std::cout << "Error" << std::endl;
                    }
                    else{
                        int64_t target_idx = it - path_id_.begin();
                        Path_t<SquareCell*> path_segment;
                        for(int i = begining_idx; i <= target_idx; i++){
                            path_segment.push_back(paths[agent.idx_][i]);
                        }
                        paths_collection.push_back(path_segment);
                        begining_idx = target_idx;
                    }
                }
            }
        }
