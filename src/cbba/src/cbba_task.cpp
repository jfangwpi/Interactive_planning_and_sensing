...
void LTLFormula::InitializeGlobalLTL(){
	...
	for (int i = 0; i < num_AP; i++){
		...
		if (type == "VISIT"){
			ap.type_ = TasksType::VISIT;
		}
		else if(type == "SEARCH"){
			ap.type_ = TasksType::SEARCH;
		}
		else{
		...
		for (int j  = bundle.size()-1; j >= 0;j--){
			if (j == bundle.size()-1 )
				...
				if (formula.sub_buchi_regions.sub_buchi_regions_Liveness_T[bundle[j]].size() == 1)
					...
					if(formula.task_type_[bundle[j]] == TasksType::VISIT){
						for (int it_sub_task_idx = formula.sub_buchi_regions.sub_buchi_regions_Liveness_T[bundle[j]].size()-1;it_sub_task_idx >= 0 ; it_sub_task_idx--)
							SubtaskFromBundle = "<> ( " + formula.sub_buchi_regions.sub_buchi_regions_Liveness_T[bundle[j]][it_sub_task_idx] + " && " + SubtaskFromBundle + ")";
					}
					else if(formula.task_type_[bundle[j]] == TasksType::SEARCH){
...
void LTLDecomposition::get_tasks_properties(LTLFormula& formula, std::map<int, TasksType> task_info){
...
cbba_Task::cbba_Task(int64_t id, std::vector<int64_t> pos, TasksType type, int64_t num_agents, std::string liveness, std::string safety, std::vector<std::string> buchi_regions):
idx_(id),
pos_(pos),
type_(type),
...
void TasksList::SetTasksFromLTL(LTLFormula Global_LTL){
    ...
    for (int i = 0; i < Global_LTL.sub_buchi_regions.sub_buchi_regions_Liveness_T.size(); i++){
        ...
        // TasksType type = TasksType::SEARCH;
...
void TasksList::SetTasksFromHotSpots(std::vector<int64_t> hotspots){
	tasks_ = {};
	for(int64_t ii = 0; ii < hotspots.size(); ii++){
		int64_t id = ii;
		std::vector<int64_t> pos = {hotspots[ii]};
		int num_agents = 1;
		TasksType type = TasksType::VISIT;

		cbba_Task task = cbba_Task(id,pos,type,num_agents,"","",{});
		tasks_.push_back(task);
	}
}
