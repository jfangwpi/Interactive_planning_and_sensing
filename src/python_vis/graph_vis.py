...
class map_vis(object):
    ...
    def entropy_trend_paths_data_handler(self, channel, data):
        ...
        with open("Entropy_trend_paths.cvs", mode="w") as csv_file:
            fieldnames = ["iter_t", "entropy1", "entropy2", "entropy3", "entropy4"]
            ...
            for c in range(len(self.entropy_paths_[0])):
                ...
                writer.writerow({"iter_t": c, "entropy1": self.entropy_paths_[0][c], "entropy2":self.entropy_paths_[1][c], "entropy3":self.entropy_paths_[2][c], "entropy4":self.entropy_paths_[3][c]})
    ...
    def vis(self):
        ...
        for agent_idx in self.agents_.keys():
            ...
            plt.text(col + 0.55, row + 0.55,"V {}".format(agent_idx), color='blue', fontsize=fontsize_dis)
        ...
        for tsk in self.tasks_.keys():
            ...
            plt.text(col + 0.55, row + 0.55,"P{}".format(tsk), color='blue', fontsize=fontsize_dis)
            x_region = np.arange(col, col + 2, 1)
            y_region = row
            plt.fill_between(x_region, y_region, y_region +1, facecolor='yellow', interpolate=True)
        ...
        if self.path_collection_:
            for idx in range(len(self.path_collection_)):
                ...
                if path != []: 
                    ...
                    # plt.text(start_vertex.pos_[1] - 0.1, start_vertex.pos_[0] - 0.1, 'S', color='blue', fontsize=fontsize_dis)

                    # Mart End Cell
                    end_id = path[-1]
                    end_vertex = self.vertex_[end_id]
                    plt.text(end_vertex.pos_[1] + 0.1, end_vertex.pos_[0] + 0.1, 'F', color='green', fontsize=20)
    ...
    def ig_vis(self):
        ...
        # Draw each cell
        for cell in self.vertex_.values():
            plt.text(cell.pos_[1] + 0.25, cell.pos_[0] + 0.25,"{}".format(round(cell.ig_,3)), color='black', fontsize=fontsize_dis)
    ...
    def visualization(self):   
        ...
        if style_dis == "combined":
            ...
            plt.suptitle("Occupancy grid map at iteration t = {}".format(self.num_fig/2 + 1), fontsize = fontsize_dis)
            ...
            plt.xticks(fontsize = fontsize_dis)
            plt.yticks(fontsize = fontsize_dis)
            plt.title("Occupancy grid map at iteration t = {}".format(self.num_fig/2 + 1), fontsize = fontsize_dis)
            ...
            plt.title("Information gain at iteration t = {}".format(self.num_fig/2), fontsize = fontsize_dis)
