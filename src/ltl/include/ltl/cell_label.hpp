...
class CellLabel{
    ...
    public:
        ...
        std::vector<int32_t> GetCellLabelID(){
            std::vector<int32_t> re_labels;
            for (auto& lb: labels_){
                int32_t current_label = lb.GetRegionLabelID();
                if(current_label != default_label_){
                    re_labels.push_back(current_label);
                }
            }
