...
{
    //======================================== SET GRID MAP =======================================//
	std::shared_ptr<SquareGrid<SquareCell>> grid = GridGraph::CreateSquareGrid<SquareCell>(10,10,1);

	// grid->SetInterestedRegionLabel(3,1);
    // grid->SetInterestedRegionLabel(99,2);
    grid->SetInterestedRegionLabel(34,2);
	// grid->SetInterestedRegionLabel(35,2);
    grid->SetInterestedRegionLabel(99,2);
    grid->SetInterestedRegionLabel(45,2);
    grid->SetInterestedRegionLabel(19,3);
    // grid->SetInterestedRegionLabel(80,4);

	std::shared_ptr<Graph_t<SquareCell *>> grid_graph = GridGraph::BuildGraphFromSquareGrid<SquareCell>(grid,false, false);

    //======================================== SET BUCHI AUTOMATON =================================//
    // Buchi Automaton
    std::vector<std::string> buchi_regions;
	// std::string ltl_formula = "[] p0 && [<>(p1 && (p2 -> <> p3)) && (<>[] p4)]";
    // std::string ltl_formula = "[] p0 && <> p2 && <> p3";
    std::string ltl_formula = " <> (p2 && (p2 -> <> p3))";
