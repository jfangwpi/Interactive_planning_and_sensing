...
def main():
    ...
    num_cases_ = 20
    training_samples_ = []
    training_datas_list_ = []
    L2_errors_ = {}
    for i in range(1,35,1):
        L2_errors_[2*i] = 0
        training_datas_list_ = []
        while len(training_datas_list_) < num_cases_:
            training_data_ = []
            while len(training_data_) < 2*i:
                t_idx = random.randint(0,99)
                if t_idx not in training_data_ and t_idx in nz_ig:
                    training_data_.append(t_idx)
            
            if training_data_ not in training_datas_list_:
                training_datas_list_.append(training_data_)
        training_samples_.append(training_datas_list_)
    ...
    try:
        if True:
            ...
            for samples_num in training_samples_:
                for sample in samples_num:
                    unknown_graph.gp(true_graph, sample)
                    true_graph.calculate_L2_error()
                    print(len(sample))
                    print(true_graph.L2_error_total_)
                    L2_errors_[len(sample)] += true_graph.L2_error_total_
            
            for t_num in L2_errors_.keys():
                L2_errors_[t_num] = L2_errors_[t_num]/num_cases_

            L2_t = sorted(L2_errors_.keys())
            L2_e = []
            for k in L2_t:
                L2_e.append(L2_errors_[k])
            print(L2_e)
            print(L2_t)
                
            plt.figure()
            plt.title("L2 error VS number of training data",fontsize=20)
            plt.plot(L2_t,L2_e,'b*-',linewidth=3)
            plt.xlim([0,70])
            plt.ylim([0,80])
            ...
            print("The L2 error along with number of training data is ")
            print(L2_errors_)
